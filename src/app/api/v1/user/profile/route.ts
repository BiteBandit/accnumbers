...
export async function GET(request: Request) {
  try {
    ...
    // 1. Verify the API key and get user_id
    ...
    // 2. Fetch user profile / auth details from Supabase Auth admin API or a profiles table
    const { data: userData, error: userError } = await supabaseAdmin.auth.admin.getUserById(apiKeyData.user_id);
    ...
    // 4. Return the exact 5-SIM style JSON structure
